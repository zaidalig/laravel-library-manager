...
<td class="text-end"><span class="table-actions"><a href="{{ route('members.show',$m) }}" class="btn btn-sm btn-outline-secondary" title="View" aria-label="View"><i class="fa-solid fa-eye"></i></a> <a href="{{ route('members.edit',$m) }}" class="btn btn-sm btn-outline-primary" title="Edit" aria-label="Edit"><i class="fa-solid fa-pen"></i></a> <button class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#deleteModal" data-url="{{ route('members.destroy',$m) }}" data-name="{{ $m->name }}"><i class="fa-solid fa-trash"></i></button></span></td></tr>
@empty<tr><td colspan="7" class="text-center py-4 text-muted">No members found.</td></tr>@endforelse
</tbody></table></div><x-table-pagination :paginator="$members" :sorts="['name'=>'Name','email'=>'Email','member_code'=>'Member code','status'=>'Status','created_at'=>'Created']" /></div>
@endsection
