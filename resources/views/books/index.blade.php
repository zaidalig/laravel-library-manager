...
<div class="col-md-2"><select name="genre_id" class="form-select form-select-sm form-select-compact"><option value="">All Genres</option>@foreach($genres as $g)<option value="{{ $g->id }}" @selected(request('genre_id')==$g->id)>{{ $g->name }}</option>@endforeach</select></div>
<div class="col-md-2"><select name="status" class="form-select form-select-sm form-select-compact"><option value="">All Status</option>@foreach(['active','inactive'] as $s)<option value="{{ $s }}" @selected(request('status')===$s)>{{ ucfirst($s) }}</option>@endforeach</select></div>
...
<td class="text-end"><span class="table-actions"><a href="{{ route('books.show',$b) }}" class="btn btn-sm btn-outline-secondary" title="View" aria-label="View"><i class="fa-solid fa-eye"></i></a> <a href="{{ route('books.edit',$b) }}" class="btn btn-sm btn-outline-primary" title="Edit" aria-label="Edit"><i class="fa-solid fa-pen"></i></a> <button class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#deleteModal" data-url="{{ route('books.destroy',$b) }}" data-name="{{ $b->title }}"><i class="fa-solid fa-trash"></i></button></span></td></tr>
@empty<tr><td colspan="8" class="text-center py-4 text-muted">No books found.</td></tr>@endforelse
</tbody></table></div><x-table-pagination :paginator="$books" :sorts="['title'=>'Title','author'=>'Author','isbn'=>'ISBN','status'=>'Status','created_at'=>'Created']" /></div>
@endsection
