...
<div class="col-md-3"><select name="status" class="form-select"><option value="">All Statuses</option>@foreach(['borrowed','returned'] as $s)<option value="{{ $s }}" @selected(request('status')===$s)>{{ ucfirst($s) }}</option>@endforeach</select></div>
<div class="col-md-3"><select name="member_id" class="form-select"><option value="">All Members</option>@foreach($members as $m)<option value="{{ $m->id }}" @selected(request('member_id')==$m->id)>{{ $m->name }}</option>@endforeach</select></div>
<div class="col-md-3"><select name="book_id" class="form-select"><option value="">All Books</option>@foreach($books as $b)<option value="{{ $b->id }}" @selected(request('book_id')==$b->id)>{{ $b->title }}</option>@endforeach</select></div>
...
</tbody></table></div>@if($loans->hasPages())<div class="card-footer bg-white">{{ $loans->links() }}</div>@endif</div>
@endsection
